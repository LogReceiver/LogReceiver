...
{
    public class SelectAllFiltersCommand : ViewModelCommandBase<IFilterSelectionControlModel>, ISelectAllFiltersCommand
    {
        public override void Execute(object parameter)
        {
            ...
            SelectAllFilters(ParentViewModel.SelectedFilterProfile.Filters);
        }

        private void SelectAllFilters(IEnumerable<PFilter> filters)
        {
            foreach (var filter in filters)
            {
                filter.IsSelected = true;
                SelectAllFilters(filter.Children);
            }
        }
    }
}
