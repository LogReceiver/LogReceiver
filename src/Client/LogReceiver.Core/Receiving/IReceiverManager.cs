...
namespace LogReceiver.Core.Receiving
{
    ...
    {
        List<IReceiverInitializer> ReceiverInitializers { get; }
        List<IReceiver> Receivers { get; } 

        void Spawn(IReceiverInitializer receiverInitializer);

        void Spawn<TReceiverInitializer, TReceiver>(TReceiverInitializer receiverInitializer, Action<TReceiver> configure)
            where TReceiverInitializer : IReceiverInitializer
            where TReceiver : IReceiver;

        void Spawn<TReceiverInitializer, TReceiver>(Action<TReceiver> configure)
            where TReceiverInitializer : IReceiverInitializer
            where TReceiver : IReceiver;
